...
def snake_start(length):
    ...
    for x in range(length-2,-1,-1):
        iterator+=1
        canvas.create_rectangle((sx-x)*10,sy*10,(sx-x)*10+10,sy*10+10,fill='black',width=0,tags='n' + str(iterator))
...
def move ():
    ...
    if nom_nom(sx,sy):
        canvas.create_rectangle(sx*10,sy*10,sx*10+10,sy*10+10,fill='black',width=0,tags='n' + str(iterator))
        opac(orientation)
...
def spawn():
    ...
    canvas.create_rectangle(pole[coord][0],pole[coord][1],pole[coord][2],pole[coord][3],width=0,fill='red',tags='food')
